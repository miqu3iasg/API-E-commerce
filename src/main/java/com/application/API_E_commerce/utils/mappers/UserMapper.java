...
public interface UserMapper {
  ...
  default List<JpaOrderEntity> ordersToJpa(List<Order> orders) {
    return orders != null ? orders.stream().map(OrderMapper.INSTANCE::toJpa).toList() : null;
  }

  @Named("ordersToDomain")
  default List<Order> ordersToDomain(List<JpaOrderEntity> jpaOrders) {
    return jpaOrders != null ? jpaOrders.stream().map(OrderMapper.INSTANCE::toDomain).toList() : null;
  }

  @Named("cartsToJpa")
  default List<JpaCartEntity> cartsToJpa(List<Cart> carts) {
    return carts != null
            ? carts.stream().map(CartMapper.INSTANCE::toJpa).toList()
            : null;
  }
  ...
  default List<Cart> cartsToDomain(List<JpaCartEntity> jpaCarts) {
    return jpaCarts != null
            ? jpaCarts.stream().map(CartMapper.INSTANCE::toDomain).toList()
            : null;
  }
}
